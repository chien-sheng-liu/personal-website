        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="flex border-b border-slate-100">
            {project.metrics.map((m, i) => (
              <div
                key={i}
                className={`flex-1 px-4 py-2.5 text-center ${i < project.metrics.length - 1 ? "border-r border-slate-100" : ""}`}
              >
                <div className="text-[8px] uppercase tracking-wider text-slate-400">{m.label}</div>
                <div className="text-sm font-bold text-[#1d1d1f]">{m.value}</div>
              </div>
            ))}
          </div>
          ...
          <div className="flex-1 px-6 py-5 overflow-y-auto" style={{ maxHeight: "400px" }}>
            ...
            {project.highlights && project.highlights.length > 0 && (
              <>
                ...
                <div className="text-[8px] uppercase tracking-wider text-slate-400 mb-2">{t.highlights}</div>
            ...
            {/* Tech tags */}
            <div className="text-[8px] uppercase tracking-wider text-slate-400 mb-2">{t.tech}</div>
          ...
          <div className="px-6 py-3 border-t border-slate-100 flex items-center justify-between">
            {project.link && project.link !== "#" ? (
              ...
              <span className="text-xs text-slate-300 italic">Private project</span>
            )}
          </div>
        </div>

        {/* ══ RIGHT STUB (desktop only) ══ */}
        <div className="hidden sm:flex relative w-[140px] shrink-0 flex-col items-center justify-center border-l border-dashed border-slate-200 bg-slate-50/50">
          {/* Perforation notches */}
          ...
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider text-center px-2">
            {project.category}
          </div>
          <div className="text-[8px] text-slate-300 mt-1 text-center px-2">
            {t.metrics}
          </div>
          <div className="mt-3 space-y-1.5">
            {project.metrics.map((m, i) => (
              <div key={i} className="text-center">
                <div className="text-xs font-bold text-[#1d1d1f]">{m.value}</div>
