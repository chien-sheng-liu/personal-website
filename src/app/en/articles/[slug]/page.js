    ...
    <div className="relative min-h-screen text-slate-800 overflow-hidden">
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-10 max-w-6xl">
        <div>
          ...
          <div className="text-sm text-slate-400 mb-6">{meta.date} · {meta.readingTime}</div>
          <article className="leading-relaxed text-slate-600 space-y-4">
            <MarkdownRenderer html={html} />
          </article>
          <div className="mt-8"><Link href="/en/articles" className="text-sky-600 hover:text-indigo-600 font-semibold">← Back to articles</Link></div>
        </div>
        <aside className="hidden lg:block sticky top-28 h-max bg-slate-50 border border-slate-200 rounded-xl p-4">
          <div className="text-sm font-semibold text-slate-900 mb-2">On this page</div>
          <nav className="space-y-1">
            {toc.map((h) => {
              const indent = Math.max(0, h.level - 2) * 12;
              return (
                <a key={h.id} href={`#${h.id}`} className="block text-slate-500 hover:text-slate-900 transition-colors" style={{ paddingLeft: `${indent}px` }}>{h.text}</a>
              );
            })}
          </nav>
        </aside>
