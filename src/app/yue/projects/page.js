...
const ProjectsPageYue = () => {
  ...
  useEffect(() => {
    fetch('/api/content/projects?locale=yue').then(r=>r.json()).then(d=>{
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <motion.p className="text-xl text-slate-600 max-w-4xl mx-auto leading-relaxed mb-12" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }}>
            以下係我喺 <span className="text-sky-500 font-semibold">數據科學</span>、<span className="text-indigo-600 font-semibold">機器學習</span> 同全端開發領域嘅精選項目，展示點樣將複雜數據問題變成商業成果。
          </motion.p>
        ...
        <motion.div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
          {projects.map((project, index) => (
            <motion.div key={index} variants={itemVariants} className="relative group overflow-hidden bg-white shadow-sm backdrop-blur-xl rounded-2xl border border-slate-200 hover:border-sky-300 transition-all duration-500 hover:shadow-lg">
              ...
              <div className="relative p-8">
                <div className="flex items-start space-x-4 mb-6">
                  <motion.div className="flex-shrink-0 p-3 bg-sky-500/10 rounded-xl border border-sky-500/20 group-hover:bg-sky-500/20 transition-all duration-300" whileHover={{ rotate: 5, scale: 1.1 }}>{project.icon}</motion.div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2 mb-1"><span className="text-sky-500">{project.categoryIcon}</span><span className="text-sm font-semibold text-sky-500">{project.category}</span></div>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.technologies.map((tech, i) => (<span key={i} className="px-3 py-1 bg-sky-500/10 border border-sky-500/20 rounded-full text-xs font-medium text-sky-500 backdrop-blur-sm">{tech}</span>))}
                </div>
                ...
                <div className="flex space-x-4">
                  ...
                  <button className="flex items-center justify-center w-12 h-12 bg-slate-100 hover:bg-indigo-50 rounded-xl transition-all duration-300 group/eye">
                    <FaEye className="text-slate-500 group-hover/eye:text-indigo-500 transition-colors duration-300" />
                  </button>
                </div>
              </div>
              <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-sky-100/50 to-transparent rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              <div className="absolute -bottom-10 left-1/2 transform -translate-x-1/2 w-32 h-10 bg-sky-100 rounded-full blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.div className="text-center" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          <div className="max-w-4xl mx-auto">
            <h3 className="text-4xl font-bold text-slate-900 mb-6">想知更多細節？</h3>
            <p className="text-xl text-slate-600 mb-8 leading-relaxed">每個項目都有佢獨特嘅挑戰同解法。我好樂意分享更多技術細節、實作過程同當中嘅得著。</p>
            <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-6">
              ...
              <div className="flex items-center space-x-2 text-slate-500"><div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div><span className="font-medium">持續更新緊</span></div>
            </div>
            <motion.div className="mt-16 pt-8 border-t border-slate-200" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.5 }}>
              <div className="flex flex-wrap justify-center items-center gap-8 text-sm text-slate-400">
                <div className="flex items-center space-x-2"><FaCode className="text-sky-500" /><span>100% 開源</span></div>
                <div className="flex items-center space-x-2"><FaStar className="text-indigo-500" /><span>實戰驗證</span></div>
                <div className="flex items-center space-x-2"><FaDatabase className="text-sky-500" /><span>完整文檔</span></div>
              </div>
            </motion.div>
          </div>
        </motion.div>
